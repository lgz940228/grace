...
//                LOGGER.error("findClassAnnotationMethods error", ignore);
            }
        }
        return methods;
    }
     ...
     * @return
     */
    public static Set<String> findAnnotationClasses(String basePackage, Class<? extends Annotation> annotation) {
        ResourcePatternResolver resourcePatternResolver = new PathMatchingResourcePatternResolver();
        MetadataReaderFactory metadataReaderFactory = new CachingMetadataReaderFactory(resourcePatternResolver);
        Set<String> clazzSet = new HashSet<String>();
        //扫描classpath下的所有文件
        ...
        try {
            ...
            for (Resource resource : resources) {
                //检查resource，这里的resource都是class
                String clzStr = loadClassName(metadataReaderFactory, resource);
                Class<?> clazz = ClassUtil.forName(clzStr, Thread.currentThread().getContextClassLoader());
                if (clazz.getAnnotation(annotation) != null) {
                    clazzSet.add(clzStr);
                }
            }
        } catch (Exception e) {
            LOGGER.error("获取标注注解的类失败,注解:" + annotation.getName(), e);
        }
        return clazzSet;
    }
}
